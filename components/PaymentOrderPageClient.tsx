...
export default function PaymentOrderPageClient({ orderId }: { orderId: string }) {
  ...
  async function submitClaim(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.set("paymentTime", paymentTime);
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">请使用微信或支付宝扫码付款，付款后提交付款信息等待管理员确认。</p>
          </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <dl className="mt-6 space-y-4 text-sm">
                ...
                <InfoRow label="创建时间" value={formatDate(order.createdAt)} />
                {order.paymentMethod ? <InfoRow label="付款方式" value={order.paymentMethod === "wechat" ? "微信" : "支付宝"} /> : null}
              ...
              {order.status === "paid" ? (
                <div className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm leading-6 text-emerald-700">
                  ...
                  管理员已确认收款，套餐已开通。你可以返回 Dashboard 查看剩余额度。
                </div>
              ) : null}
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">请按订单金额付款，付款完成后填写下方信息。页面不会自动扣款。</p>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <PaymentQr title="微信支付" src="/payment/wechat-pay.jpg" tone="green" />
                <PaymentQr title="支付宝" src="/payment/alipay-pay.jpg" tone="blue" />
              </div>

              <form onSubmit={submitClaim} className="mt-6 space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-800">付款方式</span>
                    <select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-400">
                      <option value="wechat">微信</option>
                      <option value="alipay">支付宝</option>
                    </select>
                  </label>
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-800">付款时间</span>
                    <input type="datetime-local" value={paymentTime} onChange={(event) => setPaymentTime(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-blue-400" />
                  </label>
                </div>
...
function PaymentQr({ title, src, tone }: { title: string; src: string; tone: "green" | "blue" }) {
  return (
    <div className={tone === "green" ? "rounded-2xl border border-emerald-100 bg-emerald-50 p-4" : "rounded-2xl border border-blue-100 bg-blue-50 p-4"}>
