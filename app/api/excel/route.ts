...
export async function POST(request: Request) {
  const apiKey = process.env.EXCEL_API_KEY;
  const workflowId = process.env.EXCEL_WORKFLOW_ID || "excel";

  console.log("Excel API key loaded:", Boolean(apiKey));
  console.log("Excel workflow id:", workflowId);

  if (!apiKey) {
    console.log("Dify error:", "EXCEL_API_KEY 未配置");
  ...
  try {
    ...
    const inputs = {
      excel_file: [toDifyDocument(uploadedFile.id)]
    };
    console.log("Excel request body:", inputs);
